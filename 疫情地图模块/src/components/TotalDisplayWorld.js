...
import { ShowNumBlock, ShowNumber, NumberTitle,ShowNumber_world } from './NumberDisplay';
...
export default class TotalDisplayWorld extends Component {
  ...
  render(){
    ...
    aboardData[3]=new ShowNumBlock(this.state.totalCure,'治愈人数',this.state.totalCure_c,'green'); 
    aboardData[4]=new ShowNumBlock(this.state.totalDeath,'死亡人数',this.state.totalDeath_c,'black');

    return (
      <ScrollView style={{ flex: 1 }}>
        <NumberTitle />
        {ShowNumber_world(aboardData)}
            <WhiteSpace size="lg" />
