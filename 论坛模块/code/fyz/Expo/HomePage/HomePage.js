...
class Home extends React.Component{
     ...
     */
      ...
      _getData = async (mode=0, postNum = 10)=>{
          ...
          let errDetect = 0;
          let postDate = await Promise.race([timeThreshold,DataFetch._getPostDateAsync(mode,postNum)])
          .then((value)=>{
              return value;
          })
          .catch((err)=>{
...
function HomePageStack({navigation,route}){
...
function PersonPage({navigation,route}){
    /**
     * 这里是个人主页
     */
    ...
    return(
        <View style={styles.container}>
            <Text style={styles.text}>这是个人主页</Text>
        </View>
    );
}
