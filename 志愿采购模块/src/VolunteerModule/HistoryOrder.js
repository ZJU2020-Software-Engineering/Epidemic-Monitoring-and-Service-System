...
export class HistoryOrder extends React.Component {
    constructor(props) {
        ...
        let orders = []
        this.state = {
            merchant_id: Cache.get('account'),//props.navigation.state.params.id,
            orders: orders
        };
    }
    componentDidMount() {
        GetHistoryOrderList_tenant(this.state.merchant_id).then((response)=>{this.successShow(response)});
    }
