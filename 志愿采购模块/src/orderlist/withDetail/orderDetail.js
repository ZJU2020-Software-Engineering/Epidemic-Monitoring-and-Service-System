	  ...
	  return (
	  	   <View>
		       <WingBlank>
				   ...
				   <Text style={{fontSize: 10, marginTop:5, color:'#808080',textAlign: 'center'}}>订单号: {index}</Text>
				   <Text style={{fontSize: 10, color:'#808080',textAlign: 'center'}}>{'订单关闭时间: '+order.time}</Text>
				   <WhiteSpace size='xl'/>
				   <Text style={{ fontWeight: 'bold',textAlign: 'left' }}>{order.mechantName}</Text>
				   <ScrollView><OrderTable /></ScrollView>
				   <WhiteSpace size='md'/>
				   <Text style={{fontSize: 15,textAlign: 'right'}}>{'总价: ￥'+order.totalPrice}</Text>
			   </WingBlank>
	  	   </View>
	  ); 
...
const styles = StyleSheet.create({
  ...
  tableContainer: { flex: 1, paddingTop: 10},
